                  ...
                  <TableRow key={task.id} data-state={selectedTasks.some(t => t.id === task.id) && "selected"}>
                    ...
                    <TableCell className="p-4 align-top">
                      ...
                      <div className="text-xs text-muted-foreground/80 flex items-center flex-wrap gap-x-3 gap-y-1 mt-2">
                            <div className="flex items-center gap-1.5"><User className="h-3 w-3" /><span>Por: {task.author} &bull; {format(new Date(task.createdAt as string), 'dd/MM/yy')}</span></div>
                            <div className="flex items-center gap-1.5"><Users className="h-3 w-3" /><span>Responsável: <strong className="text-foreground/90">{task.responsible}</strong></span></div>
                            <div className="flex items-center gap-1.5">{getPriorityBadge(task.priority)}</div>
                            <div className="flex items-center gap-1.5"><Calendar className="h-3 w-3" /><span>Prazo: {task.dueDate ? format(new Date(task.dueDate as string), 'dd/MM/yyyy') : 'N/A'}</span></div>
